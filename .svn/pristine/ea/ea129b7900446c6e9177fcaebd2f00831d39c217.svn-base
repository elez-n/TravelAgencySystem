...
public class FormPanel extends JPanel {
    ...
    /**
     * Vraća mapu primarnih ključeva i njihovih vrijednosti iz forme.
     * Za LinkedField polja vraća stvarni PK (selectedId), a ne display vrijednost.
     */
        ...
        for (IInputField input : inputFields) {
            if (input.isPrimary()) {
                ...
                if (input instanceof LinkedField) {
                    value = ((LinkedField) input).getLinkedId();
                } else {
                    value = input.getValue();
                }
                pkMap.put(input.getName(), value);
            }
        }
        return pkMap;
    }
    ...
    public void disableAutoincrementInputs() {
        for (IInputField input : inputFields) {
